...
# 첫 화면 고정 "FAQ TOP" 목록 (표시 라벨, /chat 전송 질문). 항목 추가는 여기 한 줄.
# 라벨의 메달 이모지는 표시용 — 전송 질문에 이모지가 섞이면 임베딩 매칭이
# 흔들릴 수 있어 전송 텍스트에서는 뺀다. 지금은 확정된 3개만 노출한다(2026-08-19,
# 추후 2개 추가 예정). 무작위로 채워 5개를 만들지 않는다.
FAQ_TOP = (
    ("🥇 원격수업, 블렌디드수업, 플립러닝 수업 운영 방법", "원격수업, 블렌디드수업, 플립러닝 수업 운영 방법"),
    ("🥈 지난학기 과목 복사 방법", "지난학기 과목 복사 방법"),
    ("🥉 학습기간 일괄 변경하기", "학습기간 일괄 변경하기"),
    # 4·5번은 준비될 때까지 숨김(2026-08-19 결정). 칩은 안 보이지만 아래 핀은
    # 살아 있어 질문을 직접 입력하면 확정 답변이 나온다 — 안정화 시험용.
    #   ("🏅 EverLec 다운로드 위치", "EverLec 다운로드 위치")   ← 노션 페이지 생성 대기
    #   ("🏅 로그인 방법", "로그인 방법")                      ← 안정화 진행 중
)

# 고정 질문 → 확정 답변 문서 핀. 검색·게이트·LLM 을 거치지 않고 노드 레지스트리의
# 해당 문서 카드(사람이 쓴 원문)를 그대로 내보낸다(backend /chat 의 핀 경로).
#
# 핀을 두는 이유: 이 두 질문은 검색이 체계적으로 빗나간다.
#   - "EverLec 다운로드 위치" → 정답은 '앱 스퀘어'(설치파일 다운로드 안내)인데,
#     제목이 "EverLec" 인 문서는 **업로드 방법** 문서라 그쪽으로 끌려간다.
#   - "로그인 방법" → 정답은 매뉴얼 '로그인 - 대시보드 유형 선택'인데, FAQ 의
#     장애 문서("로그인이 안 되는데")들과 섞인다.
# 나머지 셋은 핀하지 않는다 — 🥈🥉는 FAQ 직출력이 이미 정답을 내고, 🥇는 근거
# 문서 자체가 아직 노션에 없다(도착하면 수집·재인덱싱으로 자연 해소).
# key 는 /chat 전송 텍스트(FAQ_TOP 의 두 번째 값), 값은 (manual, doc_title).
FAQ_TOP_PINS = {
    "EverLec 다운로드 위치": ("CMS", "앱 스퀘어"),
    "로그인 방법": ("LMS", "로그인 - 대시보드 유형 선택"),
}

# 문서가 아니라 고정 문안으로 답하는 질문. 🥇는 인덱스에 근거 문서가 없고, 답은
# 자료실 링크 안내가 전부다(2026-08-19 결정). URL 은 프론트 linkify 가 클릭 가능한
# 링크로 렌더한다. 추후 노션 페이지가 생기면 이 항목을 지우고 FAQ_TOP_PINS 로
# 옮기면 된다(수집·재인덱싱 포함).
FAQ_TOP_FIXED = {
    "원격수업, 블렌디드수업, 플립러닝 수업 운영 방법": (
        "원격수업·블렌디드수업·플립러닝 수업 운영 가이드는 아래 링크에서 "
        "내려받아 확인하실 수 있습니다.\n\n"
        "https://dcms.dongseo.ac.kr/em/6a7c05f03e7d6"
    ),
}
